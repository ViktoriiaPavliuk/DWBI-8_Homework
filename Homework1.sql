...
SELECT CONCAT(N'середня ціна:', AVG(price)) COLLATE Ukrainian_100_CI_AS as 'average price'
FROM laptop;

/*TASK 1.42 */

SELECT CONCAT(N'код:', code) COLLATE Ukrainian_100_CI_AS,
CONCAT(N'модель:',  model),
CONCAT(N'швидкість процесора:', speed),
CONCAT(N'об`єм пам`яті:', ram),
CONCAT(N'розмір диску:', hd),
CONCAT(N'швидкість CD-приводу:', cd),
CONCAT(N'ціна:', price)
FROM pc;
...
CREATE TABLE [outcomes] (
	...
	[result] varchar (10) COLLATE Ukrainian_100_CI_AS NOT NULL 
);*/

UPDATE outcomes
set result = replace(result, 'OK', N'впорядку')COLLATE Ukrainian_100_CI_AS;
UPDATE outcomes
set result = replace(result, 'damaged', N'пошкодений')COLLATE Ukrainian_100_CI_AS;
UPDATE outcomes
set result = replace(result, 'sunk', N'потоплений')COLLATE Ukrainian_100_CI_AS;
SELECT *
FROM Outcomes ;
--OR
SELECT ship,battle,
	CASE
		WHEN result='OK' THEN N'впорядку'COLLATE Ukrainian_100_CI_AS
		WHEN result='damaged' THEN N'пошкодений' COLLATE Ukrainian_100_CI_AS
		ELSE N'потоплений' COLLATE Ukrainian_100_CI_AS
	END AS new_result
FROM outcomes;
...
SELECT CONCAT('from: ',RTRIM(town_from),'  to: ', town_to)
FROM trip;
...
SELECT type, COUNT (DISTINCT model) as sumOfmodels
FROM printer
GROUP BY type;
...
SELECT COUNT(REVERSE(SUBSTRING(REVERSE(RTRIM(name)), 0, CHARINDEX(' ', REVERSE(RTRIM(name))))))
FROM passenger
WHERE LEFT(REVERSE(SUBSTRING(REVERSE(RTRIM(name)), 0, CHARINDEX(' ', REVERSE(RTRIM(name))))),1)='B' OR
LEFT(REVERSE(SUBSTRING(REVERSE(RTRIM(name)), 0, CHARINDEX(' ', REVERSE(RTRIM(name))))), 1)='S' OR
LEFT(REVERSE(SUBSTRING(REVERSE(RTRIM(name)), 0, CHARINDEX(' ', REVERSE(RTRIM(name))))), 1)='A';
